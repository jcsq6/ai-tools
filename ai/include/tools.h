...
public:
    ...
    void send(thread &th, std::string_view selected, std::string_view prompt, R &&jpeg_image, std::shared_ptr<stream_handler> res)
    {
        send_impl(th, selected, prompt, std::as_bytes(std::span(jpeg_image)), std::move(res));
    }
...
private:
    ...
    void send_impl(thread &th, std::string_view selected, std::string_view prompt, std::span<const std::byte> image, std::shared_ptr<stream_handler> res);
};
